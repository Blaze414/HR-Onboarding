...
--
-- The definition had been rewritten by six migrations in turn, and the seed
-- calls it after all of them, so the newest permissions were being dropped on
-- every reset — `document.request` existed, was granted by an earlier migration,
-- and then vanished when this function ran last.
  ...
  values
    ...
    (org, 'Administrator', 'Manages the workspace. Cannot edit its own role.', 'admin',
     array[
       ...
       'department.delete','analytics.view_summary','analytics.view_full','report.view_summary',
